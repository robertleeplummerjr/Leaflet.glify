...
import {Bounds, Browser, LatLng, LatLngBounds, Map, MapOptions, Point, ResizeEvent, Util, ZoomAnimEvent} from "leaflet";
...
describe('CanvasOverlay', () => {
  ...
  describe('_reset', () => {
    describe('when canvas is set', () => {
      it('sets canvas position to 0,0', () => {
        const co = getCo();
        const map = co['_map'];
        jest.spyOn(map, 'containerPointToLayerPoint').mockReturnValue(new Point(1, 2));
        const canvas = co.canvas = document.createElement('canvas');
  ...
  describe('_animateZoom', () => {
    it('transforms canvas correctly', () => {
      ...
      co._animateZoom(e);
      expect(canvas.style.transform).toBe('translate3d(-6770px,6807px,0) scale(512)');
    });
  });
  ...
  describe('_animateZoomNoLayer', () => {
    it('transforms canvas correctly', () => {
      ...
      expect(canvas.style.transform).toBe('translate3d(-6656px,6656px,0) scale(512)');
    });
  });
  ...
  describe('_unclampedProject', () => {
    it('calls crs.transformation._transform with correct projectedPoint and scale', () => {
      ...
      const map = co['_map'];
      // @ts-expect-error
      const transform = jest.spyOn(map.options.crs.transformation, '_transform');
  ...
  describe('_unclampedLatLngBoundsToNewLayerBounds', () => {
    it('returns the correct unclamped value', () => {
      const co = getCo();
      expect(co._unclampedLatLngBoundsToNewLayerBounds(
        new LatLngBounds(
          new LatLng(1, 1),
          new LatLng(10, 10)
        ),
        ...
        .toEqual(new Bounds(new Point(-2912.822222222225, -3673.0311028545693), new Point(3640.777777777781, 2917.7852501339657)));
    });
  });
});
